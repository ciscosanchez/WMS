...
export function cartonTypeSchema(t?: T) {
  ...
  return z.object({
    ...
    code: z.string().trim().toUpperCase().min(1, msg("codeRequired", "Code is required")).max(20),
    ...
    maxWeight: z.coerce
      .number()
      .positive(msg("weightRequired", "Max weight must be greater than 0")),
    weightUnit: z.enum(WEIGHT_UNIT_OPTIONS).default("lb"),
    tareWeight: z.coerce
      .number()
      .min(0, msg("tareWeightRequired", "Tare weight cannot be negative"))
      .default(0),
    cost: z.preprocess(
